...
districts.mry<- read_rds("districts.rds")

trial<- read_rds("trial.rds")
...
ui <- fluidPage(
    ...
    sidebarLayout(
        ...
        # Main Body, in this case text output
mainPanel(
    ...
    p(""),
    p(""),
    h4("Local Preschools Map"),
    p("The following map shows Preschools/Child Care Centers that are operating in Monterey County.  Please note this is not an exhausitive list, and does not include Family Child Care homes, nor Infant/Toddler Centers.  List is derived from addresses in QRIS database."),
    p(""),
    tmapOutput("my_tmap"),
    # plotOutput("ratio"),
    # p(""),
    ...
    br(),    
    a("MCOE Early Learning Program QRIS report, 2021, iPinwheel/Hubbe Database",href="     https://monterey.ipinwheel.org/"),
    br(),
